...
class YeastarApi
{
     ...
     {
          return Cache::remember("yeastar-queue-list", 600, function () use ($token) {
               $url = config('services.yeastar.url');
               $result = Http::asJson()
                    ->withOptions(["verify" => false])
                    ->get("{$url}/queue/list", [
     ...
     {
          $token = $this->getToken();
          $queueId = $this->getQueueList($token)
          ...
          return Cache::remember("yeaster-abandoned-missed-data-{$keySlug}", 60, function () use ($token, $startDate, $endDate, $queueId) {
               $url = config('services.yeastar.url');
               $result = Http::asJson()
                    ->withOptions(["verify" => false])
